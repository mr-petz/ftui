...
export class FtuiThermostat extends FtuiElement {
  ...
  template() {
    return `
        ...
        <div class="knob-style">       
          ...
          <div class="valve"></div>
          <ftui-icon ${(!this.hasThermometer&&this.getAttribute('[humidity]')!==null ? 'name="tint" ' : '')} class="humidity-icon"></ftui-icon>
  ...
  setAngle() {
    ...
    this.knob.style.setProperty('-webkit-transform','rotate(' + gripStep + 'deg)');
    this.knob.style.setProperty('-moz-transform','rotate(' + gripStep + 'deg)');
